# binary file을 읽어서 dictionary로 변환
...
class DataReader:
    ...
    def get_train_data(self): # image binary file을 train_dict에 불러와서, data와 labels 저장
        if self.type == 'cifar-100':
            ...
            dict = unpickle(join(self.root_dir,'test.bin'))
            data=[]
            labels=[]
            name=[]
            classname=[]
            data.extend(dict[b'data'])
            labels.extend(dict[b'labels'])
            name.extend(dict[b'name'])
            classname.extend(dict[b'classname'])
            return np.array(data),np.array(labels),None, np.array(name), np.array(classname)
    ...
    def reshape_to_plot(self,data):
        ...
        return data.reshape(data.shape[0],32,32).transpose(0,1,2).astype("uint8") # transpose shape[0], 32, 32, 3 모양으로
    ...
    def plot_imgs(self,in_data,n,random=False):
        ...
        x = min(x1,y1)
        y = max(x1,y1)
        fig, ax = plt.subplots(x,y,figsize=(10,10), squeeze=False)
        i=0
        for j in range(x):
            for k in range(y):
                if random:
    ...
    def plot_img(self,data):
        if self.type !='mnist':
            ...
            data = data.reshape(data.shape[0],32,32).transpose(0,2,1).astype("uint8")
        elif self.type == 'mnist':
